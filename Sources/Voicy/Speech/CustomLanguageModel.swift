...
actor CustomLanguageModelCache {
    ...
    func configuration(for hints: [String], locale: Locale) async throws -> SFSpeechLanguageModel.Configuration {
        ...
        // Step 1: training data, one PhraseCount per expected phrase.
        let data = SFCustomLanguageModelData(
            ...
            builder: {
                for phrase in phrases {
                    SFCustomLanguageModelData.PhraseCount(phrase: phrase, count: Self.phraseCount)
                }
            }
        )
        ...
        // model to the URL in the configuration.
